...
# from .constans import ERC20_ABI
...
class Utils:
    @staticmethod
    def load_contract(address: str, web3: AsyncWeb3, abi: str) -> Optional[AsyncContract]:
        if address is None:
            return None
    ...
    async def get_decimals(self, contract_address: str, web3: AsyncWeb3) -> int:
        contract = self.load_contract(contract_address, web3, ERC20_ABI)
        ...
        while True:
            try:
                ...
                if amount > allowance_amount:
                    ...
                    raw_tx_hash = await web3.eth.send_raw_transaction(signed_tx.raw_transaction)
                    await sleep(10)
                    ...
                    logger.success(f'✔️ | Token {token} approved')
                    await sleep(5)
                    return tx_hash
                else:
                    break
